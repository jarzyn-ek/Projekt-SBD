<section class="content-header">
    <h1>
        Project
        <small><?php echo __('View'); ?></small>
    </h1>
    <ol class="breadcrumb">
        <li><a href="<?php echo $this->Url->build(['action' => 'index']); ?>"><i
                    class="fa fa-dashboard"></i> <?php echo __('Home'); ?></a></li>
    </ol>
</section>

<!-- Main content -->
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-header with-border">
                    <i class="fa fa-info"></i>
                    <h3 class="box-title"><?php echo __('Information'); ?></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <dl class="dl-horizontal">
                        <dt scope="row"><?= __('Name') ?></dt>
                        <dd><?= h($project->name) ?></dd>
                        <dt scope="row"><?= __('Id') ?></dt>
                        <dd><?= $this->Number->format($project->id) ?></dd>
                        <dt scope="row"><?= __('Expected Profit') ?></dt>
                        <dd><?= $this->Number->format($project->expected_profit) ?></dd>
                        <dt scope="row"><?= __('Start Date') ?></dt>
                        <dd><?= h($project->start_date) ?></dd>
                        <dt scope="row"><?= __('End Date') ?></dt>
                        <dd><?= h($project->end_date) ?></dd>
                    </dl>
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-header with-border">
                    <i class="fa fa-share-alt"></i>
                    <h3 class="box-title"><?= __('Workers') ?></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <?php if (!empty($project->workers)): ?>
                        <table class="table table-hover">
                            <tr>
                                <th scope="col"><?= __('Id') ?></th>
                                <th scope="col"><?= __('First Name') ?></th>
                                <th scope="col"><?= __('Last Name') ?></th>
                                <th scope="col"><?= __('Pesel') ?></th>
                                <th scope="col"><?= __('Staff Id') ?></th>
                                <th scope="col" class="actions text-center"><?= __('Actions') ?></th>
                            </tr>
                            <?php foreach ($project->workers as $workers): ?>
                                <tr>
                                    <td><?= h($workers->id) ?></td>
                                    <td><?= h($workers->first_name) ?></td>
                                    <td><?= h($workers->last_name) ?></td>
                                    <td><?= h($workers->pesel) ?></td>
                                    <td><?= h($workers->staff_id) ?></td>
                                    <td class="actions text-right">
                                        <?= $this->Html->link(__('View'), ['controller' => 'Workers', 'action' => 'view', $workers->id], ['class' => 'btn btn-info btn-xs']) ?>
                                        <?= $this->Html->link(__('Edit'), ['controller' => 'Workers', 'action' => 'edit', $workers->id], ['class' => 'btn btn-warning btn-xs']) ?>
                                        <?= $this->Form->postLink(__('Delete'), ['controller' => 'Workers', 'action' => 'delete', $workers->id], ['confirm' => __('Are you sure you want to delete # {0}?', $workers->id), 'class' => 'btn btn-danger btn-xs']) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </table>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-header with-border">
                    <i class="fa fa-share-alt"></i>
                    <h3 class="box-title"><?= __('Subcontractors') ?></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <?php if (!empty($project->subcontractors)): ?>
                        <table class="table table-hover">
                            <tr>
                                <th scope="col"><?= __('Id') ?></th>
                                <th scope="col"><?= __('Name') ?></th>
                                <th scope="col"><?= __('Service Type') ?></th>
                                <th scope="col" class="actions text-center"><?= __('Actions') ?></th>
                            </tr>
                            <?php foreach ($project->subcontractors as $subcontractors): ?>
                                <tr>
                                    <td><?= h($subcontractors->id) ?></td>
                                    <td><?= h($subcontractors->name) ?></td>
                                    <td><?= h($subcontractors->service_type) ?></td>
                                    <td class="actions text-right">
                                        <?= $this->Html->link(__('View'), ['controller' => 'Subcontractors', 'action' => 'view', $subcontractors->id], ['class' => 'btn btn-info btn-xs']) ?>
                                        <?= $this->Html->link(__('Edit'), ['controller' => 'Subcontractors', 'action' => 'edit', $subcontractors->id], ['class' => 'btn btn-warning btn-xs']) ?>
                                        <?= $this->Form->postLink(__('Delete'), ['controller' => 'Subcontractors', 'action' => 'delete', $subcontractors->id], ['confirm' => __('Are you sure you want to delete # {0}?', $subcontractors->id), 'class' => 'btn btn-danger btn-xs']) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </table>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-header with-border">
                    <i class="fa fa-share-alt"></i>
                    <h3 class="box-title"><?= __('Budgets') ?></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <?php if (!empty($project->budgets)): ?>
                        <table class="table table-hover">
                            <tr>
                                <th scope="col"><?= __('Id') ?></th>
                                <th scope="col"><?= __('Resources') ?></th>
                                <th scope="col"><?= __('Expenses') ?></th>
                                <th scope="col"><?= __('Project Id') ?></th>
                                <th scope="col"><?= __('Department Id') ?></th>
                                <th scope="col" class="actions text-center"><?= __('Actions') ?></th>
                            </tr>
                            <?php foreach ($project->budgets as $budgets): ?>
                                <tr>
                                    <td><?= h($budgets->id) ?></td>
                                    <td><?= h($budgets->resources) ?></td>
                                    <td><?= h($budgets->expenses) ?></td>
                                    <td><?= h($budgets->project_id) ?></td>
                                    <td><?= h($budgets->department_id) ?></td>
                                    <td class="actions text-right">
                                        <?= $this->Html->link(__('View'), ['controller' => 'Budgets', 'action' => 'view', $budgets->id], ['class' => 'btn btn-info btn-xs']) ?>
                                        <?= $this->Html->link(__('Edit'), ['controller' => 'Budgets', 'action' => 'edit', $budgets->id], ['class' => 'btn btn-warning btn-xs']) ?>
                                        <?= $this->Form->postLink(__('Delete'), ['controller' => 'Budgets', 'action' => 'delete', $budgets->id], ['confirm' => __('Are you sure you want to delete # {0}?', $budgets->id), 'class' => 'btn btn-danger btn-xs']) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </table>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    </div>
</section>
